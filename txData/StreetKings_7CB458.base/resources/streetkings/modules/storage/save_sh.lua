...
---@class SKSaveMechanicDocument
---@field level integer
---@field xp integer
---@field perkPoints integer
---@field perks table<string, integer>
---@field totalJobs integer
---@field totalServicePayout integer
---@field totalDynoRuns integer

---@class SKSaveWorkshopDocument
---@field hasHouse boolean
---@field unlocked boolean
---@field activeBench string|nil
---@field tuneProfile table<string, number>

---@class SKSaveFuelDocument
---@field currentFuel number
---@field maxFuel number
---@field fuelType string
---@field redFuelBoost boolean

---@class SKSaveVehicleStatsDocument
---@field favoriteCars table
---@field totalDrivingMinutes number

---@class SKSaveEconomyDocument
---@field cash integer
---@field bank integer|nil
...
---@field mechanic SKSaveMechanicDocument
---@field workshop SKSaveWorkshopDocument
---@field fuel SKSaveFuelDocument
---@field vehicleStats SKSaveVehicleStatsDocument
...
local function normalizeDocument(document)
    ...
    if type(document.progression.level) ~= 'number' or document.progression.level < 1 then
        document.progression.level = 1
    end
    ...
    if type(document.progression.tutorialCompleted) ~= 'boolean' then
        document.progression.tutorialCompleted = false
    end

    if type(document.mechanic) ~= 'table' then
        document.mechanic = {}
    end
    if type(document.mechanic.level) ~= 'number' or document.mechanic.level < 1 then
        document.mechanic.level = 1
    end
    if type(document.mechanic.xp) ~= 'number' then
        document.mechanic.xp = 0
    end
    if type(document.mechanic.perkPoints) ~= 'number' then
        document.mechanic.perkPoints = 0
    end
    if type(document.mechanic.perks) ~= 'table' then
        document.mechanic.perks = {
            instantFasten = 0,
            fineTune = 0,
            cashedUp = 0,
            seatTime = 0,
    ...
    if type(document.workshop) ~= 'table' then
        document.workshop = {}
    end
    if type(document.workshop.hasHouse) ~= 'boolean' then
        document.workshop.hasHouse = false
    end
    if type(document.workshop.unlocked) ~= 'boolean' then
        document.workshop.unlocked = false
    end
    if type(document.workshop.tools) ~= 'table' then
        document.workshop.tools = {}
    end
    if type(document.workshop.placedTools) ~= 'table' then
        document.workshop.placedTools = {}
    end
    if type(document.workshop.tuneProfile) ~= 'table' then
        document.workshop.tuneProfile = {
            rideHeight = 0,
            camber = 0,
            toe = 0,
            turboPsi = 0,
    ...
    if type(document.fuel) ~= 'table' then
        document.fuel = {}
    end
    if type(document.fuel.maxFuel) ~= 'number' then
        document.fuel.maxFuel = 100
    end
    if type(document.fuel.currentFuel) ~= 'number' then
        document.fuel.currentFuel = document.fuel.maxFuel or 100
    end
    if type(document.fuel.fuelType) ~= 'string' then
        document.fuel.fuelType = 'regular'
    end
    if type(document.fuel.redFuelBoost) ~= 'boolean' then
        document.fuel.redFuelBoost = false
    end

    if type(document.vehicleStats) ~= 'table' then
        document.vehicleStats = {
            favoriteCars = {},
            totalDrivingMinutes = 0,
        }
    else
        if type(document.vehicleStats.favoriteCars) ~= 'table' then
            document.vehicleStats.favoriteCars = {}
        end
        if type(document.vehicleStats.totalDrivingMinutes) ~= 'number' then
            document.vehicleStats.totalDrivingMinutes = 0
        end
    end
...
function SKSaves.validateDocument(document)
    ...
    assert(type(document.mechanic) == 'table', 'streetkings: invalid save document mechanic')
    requireNonNegativeInt(document.mechanic.level, 'mechanic.level')
    requireNonNegativeInt(document.mechanic.xp, 'mechanic.xp')
    requireNonNegativeInt(document.mechanic.perkPoints, 'mechanic.perkPoints')
    assert(type(document.mechanic.perks) == 'table', 'streetkings: invalid save document mechanic.perks')
    assert(type(document.workshop) == 'table', 'streetkings: invalid save document workshop')
    assert(type(document.workshop.tools) == 'table', 'streetkings: invalid save document workshop.tools')
    assert(type(document.workshop.placedTools) == 'table', 'streetkings: invalid save document workshop.placedTools')
    assert(type(document.workshop.tuneProfile) == 'table', 'streetkings: invalid save document workshop.tuneProfile')
    assert(type(document.fuel) == 'table', 'streetkings: invalid save document fuel')
    requireNonNegativeInt(document.fuel.currentFuel, 'fuel.currentFuel')
    requireNonNegativeInt(document.fuel.maxFuel, 'fuel.maxFuel')
    assert(type(document.fuel.fuelType) == 'string', 'streetkings: invalid save document fuel.fuelType')
    assert(type(document.fuel.redFuelBoost) == 'boolean', 'streetkings: invalid save document fuel.redFuelBoost')
    assert(type(document.vehicleStats) == 'table', 'streetkings: invalid save document vehicleStats')
    assert(type(document.vehicleStats.favoriteCars) == 'table', 'streetkings: invalid save document vehicleStats.favoriteCars')
    assert(type(document.vehicleStats.totalDrivingMinutes) == 'number', 'streetkings: invalid save document vehicleStats.totalDrivingMinutes')
