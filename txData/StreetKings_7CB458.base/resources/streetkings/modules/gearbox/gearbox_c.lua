...
local function applyHandlingTune(veh)
    if not DoesEntityExist(veh) or modifiedVehicles[veh] then return end
    
    modifiedVehicles[veh] = true
    modifiedCount = modifiedCount + 1
    
    if modifiedCount > 100 then
        local temp = {}
        local count = 0
        for entityId, _ in pairs(modifiedVehicles) do
            if DoesEntityExist(entityId) then
                temp[entityId] = true
                count = count + 1
            end
        end
        modifiedVehicles = temp
        modifiedCount = count
    end

    -- 1. Mass & Drive Force
    local mass = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fMass')
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fMass', mass * 1.15)
    
    local force = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fInitialDriveForce')
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fInitialDriveForce', force * 0.70)

    local maxVel = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fInitialDriveMaxFlatVel')
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fInitialDriveMaxFlatVel', maxVel * 0.70)

    -- 2. Lower Center of Gravity (Mass Offset Z)
    local com = GetVehicleHandlingVector(veh, 'CHandlingData', 'vecCentreOfMassOffset')
    SetVehicleHandlingVector(veh, 'CHandlingData', 'vecCentreOfMassOffset', vector3(com.x, com.y, com.z - 0.10))

    -- 3. Increase Anti-Roll Bar Force (reduce body roll in corners)
    local arb = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fAntiRollBarForce')
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fAntiRollBarForce', arb * 1.5)

    -- 4. Dampen Suspension Comp/Rebound (reduce bump twitchiness)
    local comp = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fSuspensionCompDamp')
    local reb = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fSuspensionReboundDamp')
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fSuspensionCompDamp', comp * 1.35)
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fSuspensionReboundDamp', reb * 1.35)

    -- 5. Increase Grip (maintain high cornering agility)
    local tMin = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMin')
    local tMax = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMax')
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMin', tMin * 1.08)
    SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMax', tMax * 1.06)

    -- 6. Apply Differential Tune
    local diffType = lib.callback.await('streetkings:shop:getActiveVehicleDifferential', false) or 'none'
    activeDiffType = diffType
    if diffType == 'lsd' then
        -- Limited Slip Differential: improves traction and stability in corners
        local currentMin = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMin')
        local currentMax = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMax')
        SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMin', currentMin * 1.10)
        SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMax', currentMax * 1.08)
    elseif diffType == 'locked' then
        -- Locked Differential (Drift Mode):
        -- 1. Reduce traction curve values significantly to allow tires to break traction easily
        local currentMin = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMin')
        local currentMax = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMax')
        SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMin', currentMin * 0.55)
        SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionCurveMax', currentMax * 0.65)
        
        -- 2. Shift traction/grip to the front axle (so rear tires slide out while front maintains steer grip)
        local tractionBias = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionBiasFront')
        if tractionBias < 0.65 then
            SetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionBiasFront', 0.65)
        end

        -- 3. Shift power/drive bias to the rear axle (making it rear-wheel-drive biased to spin tires under power)
        local driveBias = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fDriveBiasFront')
        if driveBias > 0.10 then
            SetVehicleHandlingFloat(veh, 'CHandlingData', 'fDriveBiasFront', 0.05)
        end
        
        -- 4. Increase steering lock to allow holding deeper drift angles
        local steerLimit = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fSteeringLock')
        SetVehicleHandlingFloat(veh, 'CHandlingData', 'fSteeringLock', steerLimit * 1.25)
    end
end
...
AddEventHandler('streetkings:gearbox:freeroamEnter', function()
    ...
    if cacheVeh and cacheVeh ~= 0 then
        applyHandlingTune(cacheVeh)
        if activeGearboxType ~= nil then
            vehicle     = cacheVeh
            inVeh       = true
...
exports('getActiveDifferential', function()
    return activeDiffType
end)

-- CarX-style Easy Drift Assist (for Locked Differential)
local driftSteer = 0.0

CreateThread(function()
    while true do
        local sleep = 500
        if activeDiffType == 'locked' and inVeh and vehicle ~= 0 then
            sleep = 0
            
            -- Get player raw steering input (-1.0 to 1.0)
            local playerInput = GetControlNormal(0, 59)
            
            -- Intercept default steering input
            DisableControlAction(0, 59, true)
            DisableControlAction(0, 63, true)
            DisableControlAction(0, 64, true)
            
            local localVelocity = GetEntitySpeedVector(vehicle, true)
            local forwardSpeed = localVelocity.y
            local lateralSpeed = localVelocity.x
            
            local targetSteer = playerInput
            
            if forwardSpeed > 3.0 then
                local slipRatio = lateralSpeed / forwardSpeed
                local absSlip = math.abs(slipRatio)
                
                -- If we are in a slide/slip angle
                if absSlip > 0.05 then
                    -- Assist strength: higher means stronger auto counter-steer
                    -- 1.95 is calibrated to hold slides beautifully
                    local assistStrength = 1.95
                    local assistSteer = slipRatio * assistStrength
                    
                    -- Reduce assist if steering against the drift (transitioning/correcting)
                    local isSteeringAgainst = false
                    if slipRatio < 0 and playerInput > 0.1 then
                        isSteeringAgainst = true
                    elseif slipRatio > 0 and playerInput < -0.1 then
                        isSteeringAgainst = true
                    end
                    
                    if isSteeringAgainst then
                        assistSteer = assistSteer * 0.20
                    end
                    
                    targetSteer = playerInput + assistSteer
                end
            end
            
            -- Smooth steering transition to eliminate twitching
            local dt = GetFrameTime()
            local lerpRate = 18.0 -- Fast steering response for drift controls
            driftSteer = driftSteer + (targetSteer - driftSteer) * lerpRate * dt
            driftSteer = math.max(-1.0, math.min(1.0, driftSteer))
            
            SetVehicleSteerBias(vehicle, driftSteer)
        else
            driftSteer = 0.0
            Wait(sleep)
        end
