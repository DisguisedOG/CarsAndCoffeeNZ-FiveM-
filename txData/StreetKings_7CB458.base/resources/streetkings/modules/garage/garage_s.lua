...
local function buildGarageVehicleDto(entry)
    ...
    return {
        ...
        plate = entry.plate,
        data = vehicleData,
        impoundTime = entry.impoundTime or 0,
        progression = {
...
lib.callback.register('streetkings:garage:setActiveVehicle', function(source, vehicleId)
    ...
    if entry.impoundTime and os.time() < entry.impoundTime then
        local timeRemaining = math.ceil((entry.impoundTime - os.time()) / 60)
        return { ok = false, reason = 'impounded', timeRemaining = timeRemaining }
    end
