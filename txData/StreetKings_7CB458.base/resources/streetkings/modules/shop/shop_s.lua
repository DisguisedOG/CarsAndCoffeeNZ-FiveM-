...
lib.callback.register('streetkings:shop:getActiveVehicleDifferential', function(source)
    local document = SKSaves.getDocument(source)
    if not document or not document.garage or not document.garage.activeVehicleId then return 'none' end
    local vehicleId = document.garage.activeVehicleId
    local entry = document.garage.vehicles[vehicleId]
    if not entry or not entry.data then return 'none' end
    return entry.data.differential or 'none'
end)

local VALID_DIFFERENTIAL_TYPES = { none = true, lsd = true, locked = true }

lib.callback.register('streetkings:shop:purchaseDifferential', function(source, diffType)
    if not VALID_DIFFERENTIAL_TYPES[diffType] then
        return { ok = false, reason = 'invalid_type' }
    end

    local document, vehicleId, entry = getActiveVehicleEntry(source)
    if not document or not entry then
    ...
    if diffType ~= 'none' then
        price = SKShopShared.DIFFERENTIAL_PRICES[diffType] or 0
        if cash < price then
            return { ok = false, reason = 'insufficient_funds' }
        end
    end
    ...
    document.economy.cash = cash - price
    entry.data.differential = diffType ~= 'none' and diffType or nil
