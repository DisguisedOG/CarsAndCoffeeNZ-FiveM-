        ...
        onEnter = function()
            CreateThread(function()
                ...
                if shopTypeKey == 'performance' then -- These two aren't base game mods, they're custom scripted mods.
                    ...
                    local diffType     = lib.callback.await('streetkings:shop:getActiveVehicleDifferential', false)
                    local diffIndex    = ({ lsd = 0, locked = 1 })[diffType] or -1
                    ...
                    table.insert(mods, {
                        modType        = 'differential',
                        name           = 'Differential',
                        current        = diffIndex,
                        isDifferential = true,
                        basePrice      = SKShopShared.DIFFERENTIAL_PRICES.lsd,
                        options        = {
                            { index = -1, key = 'differential:none',   name = 'Stock Open Diff', price = 0,                                     locked = false },
                            { index = 0,  key = 'differential:lsd',    name = 'L.S.D.',          price = SKShopShared.DIFFERENTIAL_PRICES.lsd,    locked = false },
                            { index = 1,  key = 'differential:locked', name = 'Locked Diff',     price = SKShopShared.DIFFERENTIAL_PRICES.locked,  locked = false },
                        },
                    })
                end
...
local function registerShopCallbacks(namespace)
    ...
    RegisterNUICallback(namespace .. ':purchaseMod', function(data, cb)
        ...
        local installTime = 8000
        local state = lib.callback.await('phone:mechanic:getState', false)
        if state and state.ok and state.perks and state.perks.instantFasten == true then
            installTime = 1000
        end

        local installSuccess = lib.progressBar({
            duration = installTime,
            label = 'Installing modification...',
            useActiveKey = true,
...
local VALID_DIFFERENTIAL_TYPES_CLIENT = { none = true, lsd = true, locked = true }

RegisterNUICallback('perfshop:purchaseDifferential', function(data, cb)
    if not VALID_DIFFERENTIAL_TYPES_CLIENT[data.type] then
        cb({ ok = false, reason = 'invalid_type' })
        return
    end

    local result = lib.callback.await('streetkings:shop:purchaseDifferential', false, data.type)
