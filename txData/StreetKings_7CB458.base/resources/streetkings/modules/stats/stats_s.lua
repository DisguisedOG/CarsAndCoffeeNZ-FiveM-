...
RegisterNetEvent('streetkings:stats:syncVehicleDriving', function(plate, modelName, displayName, seconds)
    local src = source
    if not SKSaves.hasActiveSave(src) then return end
    ...
    if type(plate) ~= 'string' or type(modelName) ~= 'string' then return end

    doc.vehicleStats = doc.vehicleStats or { favoriteCars = {}, totalDrivingMinutes = 0 }
    
    doc.vehicleStats.totalDrivingMinutes = (doc.vehicleStats.totalDrivingMinutes or 0) + (seconds / 60.0)

    local found = false
    for _, car in ipairs(doc.vehicleStats.favoriteCars) do
        if car.plate == plate or (car.modelName == modelName and car.plate == plate) then
            car.seconds = (car.seconds or 0) + seconds
            found = true
            break
        end
    end
    ...
    if not found then
        table.insert(doc.vehicleStats.favoriteCars, {
            plate = plate,
            modelName = modelName,
    ...
    table.sort(doc.vehicleStats.favoriteCars, function(a, b)
        return (a.seconds or 0) > (b.seconds or 0)
    end)

    if #doc.vehicleStats.favoriteCars > 10 then
        table.remove(doc.vehicleStats.favoriteCars)
    end

    SKSaves.write(src, 'vehicleStats', doc.vehicleStats)
end)