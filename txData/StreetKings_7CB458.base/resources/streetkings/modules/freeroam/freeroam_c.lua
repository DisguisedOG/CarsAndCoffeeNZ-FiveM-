...
SKC.RegisterGameState(GameState.FREEROAM, {
    onEnter = function(prevState)
        ...
        CreateThread(function()
            ...
            local ped = PlayerPedId()
            -- Freeze ped while teleporting and loading collision so they don't fall through
            ...
            -- Force request and wait for collision at the exit coordinates
            RequestCollisionAtCoord(spawnPos.x, spawnPos.y, spawnPos.z)
            local collisionStart = GetGameTimer()
            while not HasCollisionLoadedAroundEntity(ped) and GetGameTimer() - collisionStart < 3000 do
                Wait(50)
            end

            -- Perform a ground check, fallback to +0.8m height if ground is not found
            local spawnZ = spawnPos.z
            local foundGround, groundZ = GetGroundZFor_3dCoord(spawnPos.x, spawnPos.y, spawnPos.z + 20.0, 0)
            if foundGround then
                spawnZ = groundZ + 0.35
            else
                spawnZ = spawnPos.z + 0.8
            end

            local spawnPosChecked = vector4(spawnPos.x, spawnPos.y, spawnZ, spawnPos.w)

            TriggerServerEvent('streetkings:freeroam:enter')
            activeVehicle = requestVehicleFromServer(spawnPosChecked)
            ...
            FreezeEntityPosition(ped, false) -- Unfreeze once safely warped in the vehicle
            SetEntityMaxHealth(ped, 115)
            SetEntityHealth(ped, 115)
