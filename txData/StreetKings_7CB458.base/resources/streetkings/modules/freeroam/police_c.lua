...
-- Roaming Speed Patrol Zones
local patrolZones = {
    { name = "Downtown", coords = vector3(150.0, -1000.0, 29.0), radius = 800.0 },
    { name = "Cypress Flats", coords = vector3(950.0, -1800.0, 30.0), radius = 700.0 },
    { name = "Grand Senora Desert", coords = vector3(1500.0, 2400.0, 45.0), radius = 1500.0 },
    { name = "Paleto Bay", coords = vector3(-250.0, 6300.0, 32.0), radius = 900.0 },
    { name = "Sandy Shores", coords = vector3(1800.0, 3700.0, 33.0), radius = 1000.0 }
}
    ...
    while true do
        ...
        if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == ped then
            ...
            if speed > 120.0 then
                ...
                for _, zone in ipairs(patrolZones) do
                    ...
                    if dist <= zone.radius then
                        if math.random(100) < 10 then
                            if GetPlayerWantedLevel(PlayerId()) < 2 then
                                SetPlayerWantedLevel(PlayerId(), 2, false)
                                SetPlayerWantedLevelNow(PlayerId(), false)
                                SKNotify({ type = 'error', title = 'Patrol spotted you speeding in ' .. zone.name .. '! Pursuit initiated.' })
                            end
                        end
                        break
                    end
...
-- Impound Lot NUI callbacks
RegisterNUICallback('phone:impound:getState', function(_, cb)
    local state = lib.callback.await('phone:impound:getState', false)
    cb(state)
end)

RegisterNUICallback('phone:impound:release', function(data, cb)
    local result = lib.callback.await('phone:impound:release', false, data)
    cb(result)
end)