...
local CAMERA_PRESETS = {
    ...
    firstperson = {
        baseFov                   = 75.0,
        baseForwardOffset         = 0.15,
        sideOffset                = 0.0,
        upOffset                  = 0.0,
        pivotHeightOffset         = 0.0,
        velocityBlendRatio        = 0.05,
        velocityDirSmoothing      = 0.05,
        lateralFollowDeadzoneDeg  = 0.0,
        headingFollowDeadzoneDeg  = 0.0,
        headingFollowCatchupRatio = 1.0,
        headingFollowReturnRatio  = 1.0,
        gForceMultiplier          = 0.20,
        lateralEffectMultiplier   = 0.20,
        shakeIntensityMultiplier  = 0.30,
        gearPullbackEnabled       = true,
        gearPullbackStrength      = 0.10,
        gearPullbackAttack        = 0.020,
        gearPullbackDecay         = 0.020,
        accelSmoothing            = 0.20,
        maxSpeedForEffects        = 60.0,
        speedFovMultiplier        = 0.20,
        fovTargetInterpolation    = 1.0,
        effectInterpolation       = 0.08,
        accelPositionMultiplier   = 0.20,
        cameraRollInterpolation   = 0.30,
        cameraPitchInterpolation  = 0.30,
        neutralPitchLookInfluence = 1.0,
        lowSpeedEffects           = 15,
        brakingFovLimit           = 5.0,
    },
...
local lastThirdPersonPreset = 'relaxed'
...
local function loadKvpConfig()
    ...
    local rawTp = GetResourceKvpString('sk_cam_tp_preset')
    if rawTp and CAMERA_PRESETS[rawTp] and rawTp ~= 'firstperson' then
        lastThirdPersonPreset = rawTp
    else
        lastThirdPersonPreset = 'relaxed'
    end
    
    actionCamDisabled = GetResourceKvpString(ACTION_CAM_DISABLED_KVP) == 'true'
    for key, default in pairs(config) do
        local raw = GetResourceKvpString('sk_cam_' .. activePresetKey .. '_' .. key)
        if not raw and activePresetKey ~= 'firstperson' then
            raw = GetResourceKvpString('sk_cam_' .. key)
        end
        if raw then
...
local function getSeatBoneName(seatIndex)
    if seatIndex == -1 then
        return 'seat_dside_f'
    ...
    elseif seatIndex == 3 then
        return 'seat_dside_r1'
    elseif seatIndex == 4 then
        return 'seat_pside_r1'
    end
    return 'seat_dside_f'
end
...
tick = function(vehicle)
    ...
    local isFirstPerson = activePresetKey == 'firstperson'
    SetCamNearClip(cam, isFirstPerson and 0.12 or 0.15)
    ...
    if isFirstPerson then
        ...
        local boneName = getSeatBoneName(seatIndex)
        local dside = GetEntityBoneIndexByName(vehicle, boneName)
        if dside ~= -1 then
            local seatPos = GetWorldPositionOfEntityBone(vehicle, dside)
            local forward, right, up, pos = GetEntityMatrix(vehicle)
            -- 0.62m vertical offset from the seat bone center is usually eye level.
            baseOrigin = seatPos + (up * 0.62)
            pivotZ = baseOrigin.z
        else
            -- Fallback to ped head bone
            baseOrigin = GetPedBoneCoords(ped, 31086, 0.0, 0.0, 0.0)
            pivotZ = baseOrigin.z
        end
    else
        baseOrigin = vehPos
        pivotZ = vehPos.z + config.pivotHeightOffset
    end
    ...
    if userLookBehind then
        ...
        _camPos.x = baseOrigin.x + _camPos.x
        _camPos.y = baseOrigin.y + _camPos.y
    ...
    elseif math.abs(userYaw) > 0.1 then
        ...
        _camPos.x = baseOrigin.x + tilted.x
        _camPos.y = baseOrigin.y + tilted.y
        _camPos.z = pivotZ   + tilted.z
    end

    if _camPos.x ~= _camPos.x or _camPos.x == math.huge or _camPos.x == -math.huge then
        _camPos.x = baseOrigin.x + camFwdX * fwdOff
        _camPos.y = baseOrigin.y + camFwdY * fwdOff
        _camPos.z = pivotZ + config.upOffset
    end

    if isFirstPerson then
        applyCollision(_camPos, baseOrigin.x, baseOrigin.y, pivotZ)
    else
    ...
    local roll, basePitch, finalYaw, finalPitch
    local vehRoll  = GetEntityRoll(vehicle)
    local vehPitch = GetEntityPitch(vehicle)

    if isFirstPerson then
        -- First Person Camera Rotation: inherits vehicle's pitch, roll, and follows heading plus user inputs
        roll = clamp(-vehRoll, ROLL_MIN, ROLL_MAX)
        finalPitch = clamp(vehPitch + userTilt, PITCH_MIN, PITCH_MAX)
        
        local followYaw = (followHeadingRad * DEG_PER_RAD) - 90.0
        finalYaw = followYaw + userYaw
        finalYaw = fmod(finalYaw + 180.0, 360.0) - 180.0
    else
        -- Third Person Camera Rotation (Legacy Logic)
...
function SKCamera.setConfig(key, value)
    ...
    config[key] = value
    SetResourceKvp('sk_cam_' .. activePresetKey .. '_' .. key, tostring(value))
end
...
function SKCamera.setPreset(presetKey)
    ...
    if activePresetKey ~= 'firstperson' then
        lastThirdPersonPreset = activePresetKey
        SetResourceKvp('sk_cam_tp_preset', lastThirdPersonPreset)
    end
    
    for k, default in pairs(CAMERA_PRESETS[activePresetKey]) do
        local raw = GetResourceKvpString('sk_cam_' .. activePresetKey .. '_' .. k)
        if not raw and activePresetKey ~= 'firstperson' then
            raw = GetResourceKvpString('sk_cam_' .. k)
        end
        if raw then
            if type(default) == 'boolean' then
                config[k] = raw == 'true'
            else
                config[k] = tonumber(raw)
            end
        else
            config[k] = default
        end
...
RegisterNUICallback('phone:settings:resetCameraDefaults', function(_, cb)
    for key, value in pairs(CAMERA_PRESETS[activePresetKey]) do
        config[key] = value
        DeleteResourceKvp('sk_cam_' .. activePresetKey .. '_' .. key)
...
local function toggleFirstPerson()
    ...
    local targetPreset
    if activePresetKey == 'firstperson' then
        targetPreset = lastThirdPersonPreset
    else
        lastThirdPersonPreset = activePresetKey
        SetResourceKvp('sk_cam_tp_preset', lastThirdPersonPreset)
        targetPreset = 'firstperson'
    end

    SKCamera.setPreset(targetPreset)

    SendNUIMessage({
        type = 'settings:updateCameraPreset',
