...
local function getTimeTrialRewardConfig(activity)
    ...
    if activity.type == EventType.DELIVERY then
        -- Clone and double the delivery config cash
        local raw = SKEventsConfig.DAILY_REWARD_CONFIG.delivery
        config = {
            base = { cash = raw.base.cash * 2, playerXp = raw.base.playerXp, vehicleXp = raw.base.vehicleXp },
            goalBonus = { cash = raw.goalBonus.cash * 2, playerXp = raw.goalBonus.playerXp, vehicleXp = raw.goalBonus.vehicleXp },
            earlyBoardBonus = { cash = raw.earlyBoardBonus.cash * 2, playerXp = raw.earlyBoardBonus.playerXp, vehicleXp = raw.earlyBoardBonus.vehicleXp },
            rankedBands = {}
        }
        for i, band in ipairs(raw.rankedBands) do
            config.rankedBands[i] = {
                maxPercentile = band.maxPercentile,
                label = band.label,
                cash = band.cash * 2,
                playerXp = band.playerXp,
                vehicleXp = band.vehicleXp
            }
        end
    else
        ...
        -- Double the cash rewards for regular races
        config.base.cash = config.base.cash * 2
        config.goalBonus.cash = config.goalBonus.cash * 2
        config.earlyBoardBonus.cash = config.earlyBoardBonus.cash * 2
...
function SKEventsRewards.awardSpeedCameraXp(source, eventId, scoreValue)
    ...
    -- Double the speed camera XP
    playerXp = playerXp * 2
    vehicleXp = vehicleXp * 2

    -- Add and double cash payout for speed cameras, scaled with progression
    local cashAmount = 0
    if playerImproved then
        local baseCash = (playerFirst and 150 or 50) + math.floor(overTarget * 2.0)
        baseCash = baseCash * 2 -- Double the cash payout
        
        local level = SKProgression.getPlayerLevelFromXp(document.progression.playerXp or 0)
        local progMultiplier = 1.0 + (level - 1) * 0.05
        cashAmount = math.floor(baseCash * progMultiplier)
    end

    local payout = SKEventsRewards.applyRewardPayout(source, cashAmount, playerXp, vehicleXp)
...
---@return table
function SKEventsRewards.buildRewardPreview(activity, boardSize, entriesBeaten, percentile, source)
    ...
    -- Scale cash with progression level (+5% cash reward per level)
    if source then
        local document = SKSaves.getDocument(source)
        if document then
            local level = SKProgression.getPlayerLevelFromXp(document.progression.playerXp or 0)
            local progMultiplier = 1.0 + (level - 1) * 0.05
            preview.cash = math.floor(preview.cash * progMultiplier)
        end
    end
...
function SKEventsRewards.awardTimeTrialRun(source, eventId, vehicleClass, scoreValue, goalMet, claimAwarded, alreadyClaimed)
    ...
    if claimAwarded then
        ...
        -- Scale the total cash reward with progression level
        local document = SKSaves.getDocument(source)
        if document then
            local level = SKProgression.getPlayerLevelFromXp(document.progression.playerXp or 0)
            local progMultiplier = 1.0 + (level - 1) * 0.05
            cashAmount = math.floor(cashAmount * progMultiplier)
        end
...
function SKEventsRewards.awardMultiplayerRace(source, activity, vehicleClass, position, totalPlayers, forfeited)
    ...
    local cashAmount = bundle.cash

    -- Scale cash with progression level
    local document = SKSaves.getDocument(source)
    if document then
        local level = SKProgression.getPlayerLevelFromXp(document.progression.playerXp or 0)
        local progMultiplier = 1.0 + (level - 1) * 0.05
    ...
    local rewardData = SKEventsRewards.applyRewardPayout(source, cashAmount, bundle.playerXp, bundle.vehicleXp)
