(function (window, $) {
  ...
  function applyStyle(style) {
    ...
    if ($unit) $unit.text(lastMetric ? 'kph' : 'mph');
    if ($analogUnit) $analogUnit.text(lastMetric ? 'kph' : 'mph');
  }
  ...
  window.addEventListener('message', function (e) {
    ...
    if (d.type === 'speedometer:update') {
      ...
      if (typeof d.fuel === 'number') {
        var $fuel = $('#skSpeedoFuel');
        if ($fuel.length) {
          $fuel.css('width', d.fuel + '%');
          if (d.fuel < 20) {
            $fuel.css('background', '#ff3b30');
          } else {
            $fuel.css('background', '#ffd200');
          }
        }
      }

      // Update Red Fuel Boost Indicator
      var $redBoost = $('#skRedFuelIndicator');
      if ($redBoost.length) {
        if (d.redBoost === true) {
          $redBoost.css('display', 'flex');
        } else {
          $redBoost.css('display', 'none');
        }
      }
      ...
      if (currentStyle === 'digital') {
        ...
        $analogUnit.text(d.metric ? 'km/h' : 'mph');
