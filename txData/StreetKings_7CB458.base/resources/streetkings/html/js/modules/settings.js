(function (window) {
  ...
  window.addEventListener('message', function (e) {
    if (e.data.type === 'settings:updateCameraPreset') {
      if (activeCategoryId === 'camera') {
        SK.nui.post('phone:settings:getConfig').done(function (cfg) {
          var cat = ALL_CATEGORIES.find(function (c) { return c.id === 'camera'; });
          if (cat) { applyValues(cat, cfg); }
        });
      }
    }
  });
