(function (window) {
  ...
  function onListMouseDown(e) {
    ...
    listDrag.startX = e.pageX - els.list.offsetLeft;
    listDrag.scrollLeft = els.list.scrollLeft;
    listDrag.hasMoved = false;
    els.list.style.cursor = 'grabbing';
  }

  function onListMouseMove(e) {
    if (!listDrag.active) return;
    e.preventDefault();
    var x = e.pageX - els.list.offsetLeft;
    var walk = (x - listDrag.startX) * 1.5;
    els.list.scrollLeft = listDrag.scrollLeft - walk;
    var deltaX = (e.pageX - els.list.offsetLeft) - listDrag.startX;
    if (Math.abs(deltaX) > 5) {
      listDrag.hasMoved = true;
    }
  }

  function onListMouseUpOrLeave() {
    listDrag.active = false;
    els.list.style.cursor = 'grab';
  }
  ...
  $(function () {
    ...
    els.list.addEventListener('mousedown', onListMouseDown);
    els.list.addEventListener('mousemove', onListMouseMove);
    els.list.addEventListener('mouseup',   onListMouseUpOrLeave);
    els.list.addEventListener('mouseleave', onListMouseUpOrLeave);
