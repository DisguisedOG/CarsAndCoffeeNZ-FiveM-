(function (window) {
  ...
  function buildOptionButton(mod, opt) {
    ...
    } else if (mod.isDifferential) {
      var diffStageLabels = { '-1': 'Stock', '0': 'L.S.D.', '1': 'Locked' };
      stageText = diffStageLabels[String(opt.index)] || 'Stock';
    } else {
    ...
    if (mod.isDifferential && opt.index < 0) {
      metaText = 'Remove and restore stock open differential.';
    }
    var tagText = 'Ready';
    ...
    btn.addEventListener('click', function () {
      ...
      if (!mod.isGearbox && !mod.isNitrous && !mod.isDifferential) {
      ...
      if (opt.index === mod.current) {
        ...
        setInstallState('Free', 'No cost to remove.', 'Remove', false);
      } else if (mod.isDifferential && opt.index < 0) {
        setSelection(opt.name, 'Remove the differential upgrade and restore stock open differential.');
        setInstallState('Free', 'No cost to remove.', 'Remove', false);
      } else {
        var subtitle;
        if (mod.isGearbox) {
          ...
          subtitle = opt.index === 0
            ? 'Limited Slip Differential - improves traction and exit speed out of corners.'
            : 'Locked Differential - maximum rear lockup, perfect for drifting.';
        } else {
  ...
  var DIFFERENTIAL_INDEX_TO_TYPE = { '-1': 'none', '0': 'lsd', '1': 'locked' };
  ...
  function onBuyClick() {
    ...
    if (modTypeStr === 'differential') {
      els.buy.disabled = true;
      var diffType = DIFFERENTIAL_INDEX_TO_TYPE[String(modIndex)] || 'none';
      SK.nui.post('perfshop:purchaseDifferential', { type: diffType }).done(function (result) {
        if (!result.ok) {
          els.buy.disabled = false;
          return;
        }
